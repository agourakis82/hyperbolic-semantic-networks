...
A Lean~4 formalization (29 modules, 13 explicit axioms, with 0 \sorry{} in the 7 core
...
\begin{enumerate}
  ...
  \item \textbf{Lean~4 formalization}: 29 modules, 13 explicit axioms (3 in core ORC
...
\begin{itemize}
  ...
  \textbf{3 explicit axioms} in the core (adjacency coupling bound $W_1 \leq 2$,
  support-distance bound for the idleness measures, and specification bridge; all
  mathematically standard --- Wasserstein symmetry, the triangle inequality, and the local
  clustering bound, previously axiomatized, are now machine-checked). Eight non-core
  modules carry 65 \sorry{}-carrying declarations for ongoing formalization of spectral
  geometry, Ricci flow, random graphs, and related infrastructure.
  The sign change is formally \emph{stated} but not formally \emph{proven}.

  \item Remaining core axioms are the adjacency coupling bound
  (\texttt{wasserstein\_le\_two\_of\_adjacent}), the support-distance bound
  (\texttt{probabilityMeasure\_support\_dist\_bound}), and the specification bridge.
  Wasserstein symmetry and the triangle inequality are machine-checked in
  \texttt{WassersteinProven.lean}; the local clustering bound is machine-checked in
  \texttt{Basic.lean}; probability measure normalization in \texttt{Curvature.lean}.
...
The formalization uses Lean~4 (v4.17.0) with Mathlib (v4.17.0). It consists of 29 modules
totaling 10\,212 lines with \textbf{13 explicit axioms} (3 in the 7 core ORC-theory modules;
10 in extension modules for dynamic networks and hypercomplex algebra). The 7 core
ORC-theory modules contain \textbf{0 \sorry{}}; eight non-core modules carry 65
\sorry{}-carrying declarations (\texttt{lake build} count, 2026-07-11); the remaining 14
extension modules are \sorry{}-free.

\begin{table}[ht]
\centering
\caption{Module-by-module status. \textbf{Core}: 7 ORC-theory modules (0 \sorry{}).
\textbf{Extensions}: 14 sorry-free modules + 4 exploratory modules with stubs.
\textbf{Auxiliary}: 4 modules with proof stubs. The \sorry{} column counts sorry-carrying
declarations reported by \texttt{lake build} (verified 2026-07-11).}
\label{tab:lean}
...
\texttt{Basic.lean}          & 245   & 0 & 0 & Weighted graphs, probability measures, clustering (bounds proven) \\
\texttt{Wasserstein.lean}    & 136   & 0 & 0 & Optimal transport, couplings; symmetry + triangle proven in \texttt{WassersteinProven.lean} \\
\texttt{Curvature.lean}      & 641   & 0 & 2 & ORC definition, prob.\ normalization (proven), bounds, regime exclusivity; adjacency + support-distance bounds admitted \\
\texttt{PhaseTransition.lean}& 197   & 0 & 0 & Phase transition definition, density parameter, thresholds \\
\texttt{Bounds.lean}         & 202   & 0 & 0 & Global bounds on curvature and clustering \\
...
\texttt{WassersteinProven.lean}   & 456 & 0 & 0 & Machine-checked Wasserstein symmetry and triangle inequality \\
...
\texttt{HyperbolicSemanticNetworks.lean} & 107 & 0 & 0 & Entry point, re-exports all modules \\
...
\texttt{DepressionNetworks.lean}  & 187 & 0 & 0 & Depression speech-network phase classification \\
\texttt{OctonionGraph.lean}       & 102 & 0 & 0 & Octonion associator counting (343-triple bookkeeping) \\
\texttt{SounioVerification.lean}  & 1\,234 & 0 & 0 & Auto-generated bridge from Sounio computation traces \\
\midrule
\multicolumn{5}{l}{\itshape --- Exploratory extensions (stubs) ---} \\
\texttt{CliffordFMRI.lean}        & 510 & 14 & 0 & Clifford algebra for fMRI connectivity \\
...
\texttt{HehlFormula.lean}         & 239 &  2 & 0 & Hehl explicit ORC formula on $k$-regular graphs \\
\midrule
\multicolumn{5}{l}{\itshape --- Auxiliary with stubs ---} \\
\texttt{RandomGraph.lean}         & 1\,875 & 10 & 0 & Erd\H{o}s-R\'{e}nyi and configuration model \\
\texttt{RicciFlow.lean}           &   728  & 12 & 0 & Discrete Ricci flow on networks \\
\texttt{SpectralGeometry.lean}    &   460  & 18 & 0 & Eigenvalues, Cheeger inequality \\
\texttt{RandomGeometric.lean}     &   198  &  4 & 0 & Random geometric graph models \\
\midrule
\textbf{Total} & \textbf{10\,212} & \textbf{65} & \textbf{13} & 29 modules \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Core ORC-theory axioms (3)}: adjacency coupling bound
($\Wone(\mu_u,\mu_v) \leq 2$ for adjacent $u,v$; the $d=1$ case behind
$\kappa \in [-1,1]$), support-distance bound ($d(x,y) \leq d(u,v) + 2$ on the supports of
the idleness measures), and specification bridge (Julia implementation satisfies the formal
ORC specification). Wasserstein symmetry, the triangle inequality, and the local clustering
bound, previously axiomatized, are now machine-checked.
...
\begin{enumerate}
  ...
  \item \textbf{Complete formalization}: Replace the 13 remaining axioms with Mathlib proofs
  and eliminate the 65 \sorry{} stubs (requires ODE theory, random
...
\begin{enumerate}
  ...
  \item \textbf{Lean~4 formalization} (29 modules, 13 axioms, \textbf{0 \sorry{}} in 7
...
\begin{itemize}
  ...
  (all 29 modules, buildable with \texttt{lake build}).
