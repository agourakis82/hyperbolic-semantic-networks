...
- **Slurm functional.** Partitions: `cpu-ops` (64 CPU, node `<cpu-node>`), `gpu-orangefs` (default, 2 nodes), `all` (gres/gpu=4).
- **CPU allocatable:** `srun -p cpu-ops hostname` → job 2308 on `<cpu-node>`, rc=0. **GPU allocatable:** `srun -p gpu-orangefs --gres=gpu:1 nvidia-smi -L` → `GPU 0: NVIDIA L4`, rc=0.
...
Real Slurm: CPU job **2313** on `<cpu-node>`, GPU job **2314** on `<gpu-node>` (NVIDIA L4), ~102 s end-to-end each, `SWOW_UNIFIED_ORC_PASS`, exit 0. Acceptance (a) sign+|Δ|<0.01 and (b) κ(GPU)==κ(CPU)<1e-9 are **both met for 4/4**.
