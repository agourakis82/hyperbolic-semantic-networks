...
% Target: Journal of Complex Networks (Oxford) — March 2026
...
We analyze 16 networks spanning four categories (Table~\ref{tab:results}). The clustering threshold $\Cstar$ is derived by leave-one-out cross-validation across all 16 networks (see \S\ref{sec:theory}).

\textbf{Association networks (SWOW)}~\cite{deluca2024swow}: Small World of Words cue-response matrices for Spanish ($N=422$), English ($N=438$), Chinese ($N=465$), and Dutch ($N=500$ in LCC). Dutch SWOW is notably denser ($\kavg = 61.5$, $\eta = 7.56 > \etac$).

\textbf{Knowledge graphs (ConceptNet)}~\cite{speer2017conceptnet}: English ($N=467$) and Portuguese ($N=489$), filtered to associative weight $\geq 2.0$.

\textbf{Taxonomy networks}: English WordNet~3.1 ($N \in \{500, 2000\}$), BabelNet~5.3 Russian ($N=493$) and Arabic ($N=142$). These \emph{is-a} hierarchies are tree-like.

\textbf{Clinical}: Depression symptom network at minimum severity ($N=1634$).

\textbf{Additional association networks}: (1)~SWOW Rioplatense (Argentine/Uruguayan Spanish, $N=489$)~\cite{cabana2024swowrp}; (2)~Edinburgh Associative Thesaurus (EAT; British English, 1970s era, $N=500$)~\cite{kiss1973associative}; (3)~USF Free Association Norms (American English, $N=498$)~\cite{nelson1998usf}. These three networks test cross-cultural, cross-era, and cross-protocol generalization.

\textbf{Additional taxonomy}: OdeNet (German WordNet, $N=759$, hypernymy graph)~\cite{odenet2020}. Directly analogous to English WordNet.

\textbf{Domain extension}: FrameNet semantic frame relations (English, $N=1173$)~\cite{baker1998framenet}. Unlike WordNet, FrameNet encodes scenario-level semantic relations (Causative-of, Using, Inheritance), testing the two-parameter model on a qualitatively different semantic formalism.
...
The three predicted regimes are confirmed (Table~\ref{tab:results}): association and knowledge networks are hyperbolic ($\kbar \in [-0.32, -0.05]$, $C > 0.10$); taxonomies are Euclidean ($|\kbar| < 0.03$, $C < 0.02$); Dutch SWOW is spherical ($\kbar = +0.10$, $\eta \gg \etac$).

\subsection{Phase Boundary Crossing: Dutch SWOW}

Figure~\ref{fig:bridge} overlays all 16 semantic networks onto the random regular graph phase transition curves. The key test is Dutch SWOW: with $\eta = 7.56$ and $\etac(500) = 3.10$, the theory predicts spherical geometry. The observation $\kbar = +0.099$ confirms this---an empirical demonstration of the curvature sign-change in a real network. No prior ORC study has reported a validated phase-boundary crossing on empirical data.
...
\begin{tabular}{@{}lccc@{}}
\toprule
Split & $n$ & 2-param ($\eta,C$; $\Cstar=0.022$) & 3-param ($\eta,C,G$) \\
\midrule
...
The biological networks occupy $C \in [0.23, 0.88]$---entirely outside the semantic training range $C \in [0, 0.24]$---making extrapolation unreliable by construction. The optimal $C^*$ across all 19 networks is $0.03$ (accuracy $= 84\%$), but this fails to recover the three topology-driven biological spherical cases because they have $C > 0.03$ yet moderate $\eta$. No scalar threshold on $(\eta{}, C, G)$ simultaneously classifies all semantic and biological networks correctly without adding domain identity as an implicit fourth feature.

\paragraph{Domain-shift finding.}
The two-parameter model is \emph{domain-specific}, not universal. It was trained on word association and knowledge graph networks with $C \in [0, 0.24]$, and correctly classifies $13/16$ such networks (81\%, LOO), but degrades to $25\%$ on biological networks with $C \in [0.23, 0.88]$. This is the expected behavior of any model trained outside its feature support. Correcting it requires either: (a) biological training data to recalibrate $C^*_{\mathrm{bio}} \approx 0.22$, which fixes C.\ elegans neural ($C=0.46 > C^*_{\mathrm{bio}}$) and C.\ elegans metabolic ($C=0.71$) at the cost of mislabeling clustered hyperbolic semantic networks if applied universally; or (b) a network-type-aware classifier that applies domain-specific thresholds. Both paths require additional validation data.
...
\paragraph{Fixed-point convergence.}
The flow converges rapidly: ConceptNet EN's per-edge curvature variance drops $10{,}000\times$ (from $\sigma_\kappa = 0.130$ to $10^{-5}$) by $t \approx 20$; SWOW Dutch drops $3{,}200\times$ over the same interval.
...
% ============================================================
\subsection{LLM vs.\ Human Semantic Geometry}
\label{sec:discovery_l}

Using the LWOW dataset (\S\ref{sec:methods}), we compared ORC across human and LLM association networks on matched 438-node vocabularies (Table~\ref{tab:discovery_l}).
...
\toprule
Network & $N$ & $\eta$ & $C$ & $\kbar$ & Geometry \\
\midrule
SWOW-EN (Human) & 438 & 0.020 & 0.128 & $-0.137$ & Hyperbolic \\
LWOW-Haiku & 344 & 0.029 & 0.189 & $-0.089$ & Hyperbolic \\
LWOW-Mistral & 418 & 0.164 & 0.182 & $-0.224$ & Hyperbolic \\
LWOW-Llama3 & 417 & 0.506 & 0.130 & $-0.140$ & Hyperbolic \\
\bottomrule
\end{tabular}
\end{table}
