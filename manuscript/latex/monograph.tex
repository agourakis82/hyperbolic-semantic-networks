...
\end{itemize}
Mean slope $0.417 \pm 0.026$ across $k$; all slopes $p \ll 0.001$. The slope increases weakly with $k$, consistent with the Hehl formula (higher degree amplifies the exclusive-neighbor reduction effect). This universality across degree upgrades Theorem~2 from a single-$k$ observation to a robust empirical law.~[EMPIRICAL]

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{../../figures/monograph/figure12_watts_strogatz.pdf}
\caption{Theorem~2 validated across $k \in \{4,6,8\}$: mean ORC $\kbar$ vs.\ clustering coefficient $C$ for Watts--Strogatz graphs ($N=200$, 10 $\beta$-values $\in [0.001, 1.0]$, exact LP ORC). Each colour/marker denotes a degree value; lines are linear fits. All three slopes are positive ($+0.389$, $+0.422$, $+0.441$ for $k=4,6,8$; $R^2 > 0.993$ in all cases), confirming strict monotonicity $\partial\kbar/\partial C > 0$ at fixed $\eta$ across degree values.}
\label{fig:ws_sweep}
\end{figure}
...
\begin{table}[t]
\centering
\caption{Exact LP Ollivier--Ricci curvature for 16 semantic networks. Networks above the horizontal rule constitute the \emph{training set} (used to derive $\Cstar$); those below constitute the \emph{held-out test set}. $\eta = \kavg^2/N$; $C$ = global clustering coefficient; $\kbar$ = mean edge curvature. Pred.\ = model prediction; Corr.\ = correct classification.}
\label{tab:results}
\small
\begin{tabular}{@{}lllrrrrrll@{}}
\toprule
Network & Category & Language & $N$ & $C$ & $\eta$ & $\kbar$ & Pred. & Corr. \\
\midrule
\multicolumn{9}{@{}l}{\textit{Training set (11 networks; $\Cstar = 0.05$ fitted here)}} \\
\midrule
SWOW Spanish & Assoc. & Spanish & 422 & 0.136 & 0.017 & $-0.068$ & Hyp. & \checkmark \\
SWOW English & Assoc. & English & 438 & 0.128 & 0.020 & $-0.137$ & Hyp. & \checkmark \\
SWOW Chinese & Assoc. & Chinese & 465 & 0.173 & 0.023 & $-0.144$ & Hyp. & \checkmark \\
SWOW Dutch & Assoc. & Dutch & 500 & 0.238 & 7.558 & $+0.099$ & \textbf{Sph.} & \checkmark \\
ConceptNet EN & Knowledge & English & 467 & 0.108 & 0.223 & $-0.233$ & Hyp. & \checkmark \\
ConceptNet PT & Knowledge & Portuguese & 489 & 0.106 & 0.085 & $-0.236$ & Hyp. & \checkmark \\
WordNet EN & Taxonomy & English & 500 & 0.013 & 0.009 & $-0.002$ & Euc. & \checkmark \\
WordNet EN-2K & Taxonomy & English & 2000 & 0.002 & 0.002 & $-0.005$ & Euc. & \checkmark \\
BabelNet RU & Taxonomy & Russian & 493 & 0.001 & 0.009 & $-0.030$ & Euc. & \checkmark \\
BabelNet AR & Taxonomy & Arabic & 142 & 0.000 & 0.032 & $-0.012$ & Euc. & \checkmark \\
Depression & Clinical & English & 1634 & 0.159 & 0.118 & $-0.130$ & Hyp. & \checkmark \\
\midrule
\multicolumn{9}{@{}l}{\textit{Held-out test set (5 networks; $\Cstar = 0.05$ applied without re-fitting)}} \\
\midrule
SWOW Rioplatense & Assoc. & Arg.\ Spanish & 489 & 0.154 & 0.040 & $-0.264$ & Hyp. & \checkmark \\
EAT & Assoc. & British Eng. & 500 & 0.143 & 1.851 & $-0.046$ & Hyp. & \checkmark \\
USF Free Assoc. & Assoc. & American Eng. & 498 & 0.143 & 0.062 & $-0.321$ & Hyp. & \checkmark \\
WordNet DE & Taxonomy & German & 759 & 0.022 & 0.006 & $-0.023$ & Euc. & \checkmark \\
FrameNet & Frames & English & 1173 & 0.045 & 0.009 & $-0.202$ & Euc.? & \textsection \\
\bottomrule
\multicolumn{9}{@{}l}{\footnotesize\textsection FrameNet: $C=0.045 \approx \Cstar$; borderline case; predicted Euclidean but measured Hyperbolic ($\kbar=-0.202$).}\\
\multicolumn{9}{@{}l}{\footnotesize Test set accuracy: 4/5 correct by ($\eta$, $C$) rule; FrameNet borderline refines $\Cstar$ estimate.}\\
\end{tabular}
\end{table}
...
The classifier was trained on semantic networks with $C \in [0, 0.24]$ and $\eta \in [0.009, 7.56]$. Biological networks operate at $C \in [0.23, 0.88]$---outside the training distribution. Three of four mispredictions arise because $C^* = 0.05$ is too low a threshold for a network class where even the sparsest biological graph ($C = 0.226$) exceeds the entire semantic clustering range. The appropriate biological threshold appears to be $C^*_{\mathrm{bio}} \lesssim 0.20$: at $C = 0.226$, positive curvature already emerges via hub topology. Formally, the Theorem~2 result $\partial\kbar/\partial C > 0$ implies that the hyperbolic$\to$spherical crossing in $C$-space occurs at a network-type-dependent $C^*(\eta, \mathrm{type})$, not a universal constant.
...
Finite-size scaling & $\etac(N) \sim \etac^\infty - a/\sqrt{N}$, $R^2 = 0.995$ \\
\bottomrule
\end{tabular}
\end{table}
