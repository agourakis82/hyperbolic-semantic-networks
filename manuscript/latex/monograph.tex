...
The depression network ($\kbar = -0.130$, $\Dksem = +0.388$) demonstrates the theory extends to clinical symptom networks. The full severity spectrum is now analysed: minimum ($N=1634$, $\eta=0.118$, $\kbar=-0.130$), mild ($N=3089$, $\eta=0.215$, $\kbar=-0.074$), moderate ($N=2238$, $\eta=0.207$, $\kbar=-0.087$), and severe ($N=2685$, $\eta=0.214$, $\kbar=-0.078$). All four severity levels are hyperbolic, but minimum-severity is markedly more hyperbolic than the higher-severity levels. This is consistent with the phase curve: minimum severity adds fewer comorbid symptom links (lower $\eta = 0.118$ vs.\ $\eta \approx 0.21$ for all other levels), placing it further from the phase boundary and deeper into the hyperbolic regime. Increasing severity raises $\eta$ and pushes $\kbar$ toward zero---a \emph{geometric signature of comorbidity density}. The minimum-to-mild jump ($\Delta\kbar \approx +0.056$, $\Delta\eta \approx +0.097$) may reflect a clinically meaningful connectivity threshold and warrants prospective validation.
...
\begin{enumerate}
    ...
    \item \textbf{Depression}: Only minimum severity computed; full trajectory pending.
