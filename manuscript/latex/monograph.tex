...
% ============================================================
\section{Discovery J: HPO Ontology is Hyperbolic; Disease Co-Occurrence is Spherical}
...
\paragraph{Predictions from $\eta$-theory (no free parameters).}
A tree on $N$ nodes has $\langle k \rangle \approx 2$ and $\eta \approx 4/N \to 0$ as $N \to \infty$. The HPO IS-A graph (N=19,389, $\langle k \rangle=2.44$) has $\eta = 0.0003 \ll \eta_c = 3.75$: \textbf{Hyperbolic} predicted. For the co-occurrence network (top-600 diseases, $\geq 3$ shared phenotypes), the actual density is far higher than anticipated: $\langle k \rangle = 489$, giving $\eta = 399 \gg \eta_c$: \textbf{Spherical} predicted. The $\eta$ range across both networks spans six orders of magnitude.

\paragraph{Measured ORC (exact LP, $\alpha=0.5$, subsampled edges).}
\begin{center}
\small
\begin{tabular}{@{}lrrrcccc@{}}
\toprule
Network & $N$ & $E$ & $\langle k \rangle$ & $\eta$ & $\kbar$ & Predicted & Measured \\
\midrule
...
\end{tabular}
\end{center}
\noindent Exact LP ($\alpha=0.5$, HiGHS tol $10^{-7}$): 400 sampled edges for HPO IS-A; 2000 for co-occurrence. Striking asymmetry: $\eta_{\text{cooc}} = 398.7$ vs $\eta_{\text{HPO}} = 0.0003$---a factor of $10^6$.
...
\begin{itemize}
  \item The IS-A hierarchy imposes a \emph{strict parent-child tree}---the information-theoretic ideal for hierarchical knowledge organisation. With $\eta = 0.0003$ (a factor of 12,500 below $\eta_c$), tree graphs have minimal $\eta$, maximal negative ORC ($\kbar = -0.11$), and logarithmically growing path lengths.
  \item Disease co-occurrence is driven by \emph{clinical comorbidity}---diseases cluster in phenotypic syndromes (metabolic, neurological, skeletal), creating near-complete cliques ($C = 0.908$, $\eta = 399$, $\kbar = +0.43$). This is the most spherical network in the entire study.
\end{itemize}
This is a \emph{within-domain control}: the same HPO database produces both the most hyperbolic and most spherical networks in this study, purely by changing the edge-formation rule (is-a hierarchy vs.\ co-annotation count). This echoes Discovery M (aging comorbidity networks are spherical) and confirms that geometry encodes \emph{how knowledge is organised}, not what it is about.~[EMPIRICAL]

\paragraph{Formal verification (Lean 4, Group 17).}
The parameter bounds $\eta_{\text{HPO}} < \eta_c$ and $\kbar_{\text{HPO}} < 0 < \kbar_{\text{cooc}}$ are verified as numerical theorems in \texttt{SounioVerification.lean} (Group 17, 8 theorems, 0 sorry). The \texttt{discovery\_j\_geometry\_dichotomy} theorem captures the full claim; \texttt{hpo\_cooc\_curvature\_separation} proves the strict separation $\kbar_{\text{HPO}} < \kbar_{\text{cooc}}$.~[FORMALIZED]
...
% ============================================================
\section{Discovery N: Sedenion Mandelbrot Orbit Features Discriminate ASD from ADHD Brain Topology}
\label{sec:sedenion_dual}
% ============================================================

\subsection{Motivation: Beyond Quaternionic ORC}

Quaternionic ORC (Section~\ref{sec:lean}) encodes multi-modal brain networks as a single $\kappa_\mathbb{H} \in \mathbb{H}$, capturing geometry at the edge level. A complementary question is whether the \emph{intrinsic algebraic structure} of the sedenion algebra $\mathbb{S} = \mathbb{R}^{16}$---specifically, its zero divisors---can encode brain network topology as fractal orbit features. Sedenions are the eighth Cayley--Dickson doubling of $\mathbb{R}$, obtained by extending octonions, and are the first algebra in the tower to contain non-trivial zero divisors~\cite{moreno1998zero}: elements $a, b \neq 0$ such that $a \cdot b = 0$.

\paragraph{Proposition 2.5 (Moreno, 1997).}
The sedenion pair $(e_1 + e_{10})(e_4 - e_{15}) = 0$, where $e_i$ denotes the $i$-th standard basis element of $\mathbb{S}$.

We exploit this algebraic singularity to define a \emph{Mandelbrot orbit in $\mathbb{S}$}:
\begin{equation}
z_{n+1} = z_n^2 + c, \quad z_0, c \in \mathbb{S},
\label{eq:sed_mandelbrot}
\end{equation}
where multiplication is the Cayley--Dickson product. We encode a brain graph $G$ into $c$ (spectral and structural statistics) and $z_0$ (degree moments), then extract 16 orbit features from the trajectory $\{z_n\}$.

\subsection{Sedenion Mandelbrot Orbit Features}

Given a graph $G$ with $N$ nodes and mean degree $\langle k \rangle$, the encoding maps:
\begin{align}
c &\leftarrow [\lambda_1,\ldots,\lambda_8,\; \eta, \text{density}, C, \log(1{+}\langle k\rangle)/\log(1{+}N{-}1), \ldots] \in \mathbb{S}, \\
z_0 &\leftarrow [\langle k\rangle/N,\; \sqrt{\langle k\rangle/N},\; \langle k\rangle^2/N^2,\; 0,\ldots,0] \in \mathbb{S},
\end{align}
where $\lambda_i$ are the first 8 normalised Laplacian eigenvalues and $\eta = \langle k\rangle^2/N$.

We run orbit~\eqref{eq:sed_mandelbrot} for at most $T=100$ iterations (threshold $\|z_n\| > 10$) and extract:
\begin{enumerate}
...
\item \textbf{Zero-divisor proximity} $J_n = \tfrac{1}{2}(|z_n[2]{+}z_n[11]| + |z_n[5]{-}z_n[16]|)/\sqrt{2}$ --- projection onto the Prop.~2.5 zero-divisor subspace.
...
\textit{Proof.} $H_{ij} = H_{ji}$ follows immediately from Schwarz's theorem (equality of mixed partial derivatives), since $\|z_n\|^2$ is a smooth real-valued function of $c \in \mathbb{R}^{16}$. No sedenion-specific structure is required. $\square$~[FORMALIZED: Lean Group~16, \texttt{hessian\_symmetry\_theorem\_4\_6}]

Numerically, we confirm $\max_{i,j}|H_{ij} - H_{ji}| < 10^{-14}$ across all tested $k$ values (Figure~\ref{fig:phase10_hessian}).

\subsection{Dual Analysis: ORC + Sedenion Features}

We combine two feature representations for ASD-vs-ADHD classification:
\begin{itemize}
\item \textbf{ORC features} (dim 8): $[\bar\kappa_\mathbb{R}, \bar\kappa_\mathbb{I}, \bar\kappa_\mathbb{J}, \bar\kappa_\mathbb{K}, |\bar\kappa_\mathbb{H}|, \eta, \eta/\eta_c, \sigma_\text{CI}]$, from the quaternionic BrainORC pipeline (Phase~9).
...
\centering
\small
\caption{ASD vs.\ ADHD classification on synthetic k-regular graphs ($N=39$, $n=50$ per class). Sedenion-only AUROC confirms independent discriminative power; ORC ceiling at 1.000 reflects exact $\eta$-separability. Dual $\geq$ ORC-only in all configurations.~[EMPIRICAL]}
...
Key findings (Figure~\ref{fig:phase10_orbits}--\ref{fig:phase10_eta}):
\begin{enumerate}
...
\caption{Left: Sedenion orbit norm $\|z_n\|$ for ASD ($k=4$, bounded) and ADHD ($k=16$, diverging). Right: Zero-divisor proximity $J_n$ trajectory. Both panels illustrate that phase geometry determines sedenion orbit structure.~[EMPIRICAL]}
...
\caption{Left: $\max_{i,j}|H_{ij}-H_{ji}|$ confirming Theorem~4.6 (Hessian symmetry) to machine precision across all $k$. Right: ORC vs.\ sedenion feature correlation heatmap; low cross-block correlations confirm complementarity.~[EMPIRICAL]}
...
\caption{Left: Sedenion Mandelbrot escape time in the $c[e_0]$--$c[e_1]$ plane; ASD/ADHD operating points marked. Right: Sedenion discriminability vs.\ $\eta$ with $\eta_c(N{=}100)$ phase boundary; step increase at $\eta_c$ mirrors the ORC phase transition.~[EMPIRICAL]}
...
Lean~4 Group~16 (module \texttt{SounioVerification.lean}) provides formal proofs (0 \texttt{sorry}) for:
\begin{itemize}
...
The dual analysis is implemented in three cross-validating layers:
\begin{enumerate}
\item \textbf{Python reference} (\texttt{code/analysis/sedenion\_mandelbrot.py}): Cayley--Dickson recursion via \texttt{numpy}; 10/10 gates pass.
\item \textbf{Sounio kernel} (\texttt{experiments/10\_sedenion\_mandelbrot/sedenion\_orbit.sio}): production orbit computation with sedenion multiply verified against Python.
\item \textbf{F\# production} (\texttt{fsharp/BrainORC/SedenionFeatures.fs}, \texttt{DualAnalysis.fs}): pure F\# Gaussian Na\"ive Bayes classifier; 0 warnings, \texttt{dotnet build} clean; 6/6 algebra gates pass at runtime.
\end{enumerate}
